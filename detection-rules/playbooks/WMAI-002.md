...
```text
event.dataset:"watchmyai.events" and event.action:"policy_violation" and watchmyai.policy.violation.type:("execution_outside_approved_workspace" or "path_scope") and watchmyai.tool.category:"file"
```

## Triage

1. Confirm the alert's stable rule ID is `WMAI-002` and record its source event time.
2. Inspect the fields below and preserve the original source events:

- `event.action`
- `event.dataset`
- `watchmyai.policy.violation.type`
- `watchmyai.tool.category`
...
Follow the [public verification guide](../../docs/VERIFICATION.md) for `SCN-WMAI-002`. A passing validation requires a current alert from `WMAI-002`; historical or uncorrelated alerts do not count.
