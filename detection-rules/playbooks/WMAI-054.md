...
```text
event.dataset:"watchmyai.events" and event.action:"tool_request" and (watchmyai.tool.arguments.command:(*id_rsa* or *id_ed25519* or *id_ecdsa* or *authorized_keys* or *.ssh*) or watchmyai.tool.arguments.file_path:(*id_rsa* or *id_ed25519* or *.ssh*) or process.command_line:(*id_rsa* or *.ssh*))
```

## Triage

1. Confirm the alert's stable rule ID is `WMAI-054` and record its source event time.
...
Follow the [public verification guide](../../docs/VERIFICATION.md) for `SCN-WMAI-054`. A passing validation requires a current alert from `WMAI-054`; historical or uncorrelated alerts do not count.
