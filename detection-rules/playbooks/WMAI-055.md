...
```text
event.dataset:"watchmyai.events" and event.action:"tool_request" and watchmyai.tool.arguments.command:*git* and watchmyai.tool.arguments.command:(*clone* or *push*)
```

## Triage

1. Confirm the alert's stable rule ID is `WMAI-055` and record its source event time.
...
Follow the [public verification guide](../../docs/VERIFICATION.md) for `SCN-WMAI-055`. A passing validation requires a current alert from `WMAI-055`; historical or uncorrelated alerts do not count.
