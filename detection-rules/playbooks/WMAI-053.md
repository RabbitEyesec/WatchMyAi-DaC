...
```text
event.dataset:"watchmyai.events" and event.action:"tool_request" and (watchmyai.tool.arguments.command:(*ssh* or *scp* or *sftp* or *New-SSHSession*) or process.command_line:(*ssh* or *scp*))
```

## Triage

1. Confirm the alert's stable rule ID is `WMAI-053` and record its source event time.
...
Follow the [public verification guide](../../docs/VERIFICATION.md) for `SCN-WMAI-053`. A passing validation requires a current alert from `WMAI-053`; historical or uncorrelated alerts do not count.
