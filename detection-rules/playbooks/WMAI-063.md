...
```text
event.dataset:"watchmyai.events" and event.action:"tool_request" and (watchmyai.tool.arguments.command:(*Recurse* or *-rf* or *-fr*) or process.command_line:(*Recurse* or *-rf*))
```

## Triage

1. Confirm the alert's stable rule ID is `WMAI-063` and record its source event time.
...
Follow the [public verification guide](../../docs/VERIFICATION.md) for `SCN-WMAI-063`. A passing validation requires a current alert from `WMAI-063`; historical or uncorrelated alerts do not count.
