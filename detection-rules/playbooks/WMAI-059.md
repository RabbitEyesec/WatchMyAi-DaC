...
```text
event.dataset:"watchmyai.events" and event.action:"tool_request" and (watchmyai.tool.arguments.command:(*gcloud* or *aws* or *azure* or *kubelogin*) or process.command_line:(*gcloud* or *aws*))
```

## Triage

1. Confirm the alert's stable rule ID is `WMAI-059` and record its source event time.
...
Follow the [public verification guide](../../docs/VERIFICATION.md) for `SCN-WMAI-059`. A passing validation requires a current alert from `WMAI-059`; historical or uncorrelated alerts do not count.
