...
```text
event.dataset:"watchmyai.events" and event.action:"policy_violation"
```

Threshold: 5 events grouped by `watchmyai.session.id`.

## Triage

1. Confirm the alert's stable rule ID is `WMAI-048` and record its source event time.
...
Follow the [public verification guide](../../docs/VERIFICATION.md) for `SCN-WMAI-048`. A passing validation requires a current alert from `WMAI-048`; historical or uncorrelated alerts do not count.
