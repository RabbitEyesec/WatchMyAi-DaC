...
```text
event.dataset:"watchmyai.events" and event.action:"tool_request" and (watchmyai.tool.arguments.command:(*docker* or *docker-compose*) or process.command_line:*docker*)
```

## Triage

1. Confirm the alert's stable rule ID is `WMAI-060` and record its source event time.
...
Follow the [public verification guide](../../docs/VERIFICATION.md) for `SCN-WMAI-060`. A passing validation requires a current alert from `WMAI-060`; historical or uncorrelated alerts do not count.
