...
```text
event.dataset:"watchmyai.events" and event.action:"tool_request" and (watchmyai.tool.arguments.command:(*Invoke-WebRequest* or *Invoke-RestMethod* or *curl* or *wget* or *WebClient* or *DownloadString* or *DownloadFile*) or process.command_line:(*curl* or *wget* or *Invoke-WebRequest*))
```

## Triage

1. Confirm the alert's stable rule ID is `WMAI-030` and record its source event time.
...
Follow the [public verification guide](../../docs/VERIFICATION.md) for `SCN-WMAI-030`. A passing validation requires a current alert from `WMAI-030`; historical or uncorrelated alerts do not count.
