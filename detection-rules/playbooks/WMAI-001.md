...
```text
event.dataset:"watchmyai.events" and event.action:"policy_violation" and watchmyai.policy.violation.type:"ai_access_outside_approved_workspace"
```

## Triage

1. Confirm the alert's stable rule ID is `WMAI-001` and record its source event time.
...
Follow the [public verification guide](../../docs/VERIFICATION.md) for `SCN-WMAI-001`. A passing validation requires a current alert from `WMAI-001`; historical or uncorrelated alerts do not count.
